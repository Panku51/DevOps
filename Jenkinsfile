pipeline {
    ...
    stages {
        ...
        stage('Create Grafana Dashboard') {
            ...
            steps {
                bat 'docker run -d -p 3001:3001 --name grafana grafana/grafana'
//                 bat 'timeout /t 10 /nobreak'
                bat "curl -X POST -H \"Content-Type: application/json\" \
                    ...
                    http://admin:${API_KEY}@172.24.64.1:3000/api/datasources"
                bat "curl -X POST -H \"Content-Type: application/json\" \
                    ...
                    http://admin:${API_KEY}@172.24.64.1:3000/api/dashboards/db"
            }
        }
        stage('Build') {
            steps {
